...
drivers, robot drivers, or `arm_api2` itself. Camera and robot data are
expected over ROS 2/DDS from other host processes or containers.

`arm_api2_msgs` (just the interface definitions, not `arm_api2`) *is*
vendored in at build time: `mp_wrapper_ros`'s `hamer_live_traj_plan_3d`
imports `arm_api2_msgs.action` directly, and it's not a declared
`package.xml` dependency (looked up by string via `rosidl` at runtime), so
leaving it out lets the build succeed while the node crashes at launch.
