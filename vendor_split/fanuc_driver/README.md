...
`fanuc_driver` has git submodules under `fanuc_libs/dependencies/`
(`sockpp`, `reflect-cpp`, `readerwriterqueue`, `yaml-cpp`). The Dockerfile
clones with `--recurse-submodules`; a plain clone will fail at
`colcon build` with `repository '.../dependencies/sockpp' does not exist`.

`ros2_fanuc_interface` (`fanuc_eth_ip`) needs
[EIPScanner](https://github.com/nimbuscontrols/EIPScanner) installed to
`/usr/local` before `colcon build`, or you get
`fatal error: EIPScanner/MessageRouter.h: No such file or directory`.
