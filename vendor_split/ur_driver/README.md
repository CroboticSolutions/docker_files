...
**Note on `serial`:** `robotiq_driver` links `serial::serial`. That imported
target comes from `tylerjw/serial@ros2` (`ament_export_targets`). The
RoverRobotics `serial-ros2` fork builds as a colcon package but does not
export the namespaced target, so `colcon build` fails at `robotiq_driver`.
